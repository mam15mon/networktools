			...
			<div class="p-4 space-y-6 text-sm leading-relaxed text-(--ui-text-muted)">
				...
				<section>
					<UPageList>
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>Excel 列名</UKbd><span class="text-sm">会被映射成同名变量，如列 <code>device_hostname</code> → <code v-pre>{{ device_hostname }}</code></span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>自定义变量</UKbd><span class="text-sm">用 <code v-pre>{% set core_id = device.id + 1 %}</code> 定义，仅在当前作用域可用</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>组合访问</UKbd><span class="text-sm">多层数据用点号：<code v-pre>{{ device.ip.loopback }}</code></span>
							</div>
						</div>
						<TemplateBatchCodeSnippet class="mt-2" :code="snippets.variableBasics" />
						<UAlert color="warning" variant="soft" class="mt-3">
							<template #description>
								变量名仅支持 <code>a-z</code> / <code>A-Z</code> / <code>0-9</code> / <code>_</code>，<strong>不要</strong>出现中文字符、加减号、空格或其它符号，否则解析失败。
							</template>
						</UAlert>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>upper</UKbd><span class="text-sm">全部大写，常用于接口描述</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>capitalize</UKbd><span class="text-sm">句首大写，如 "core router" → "Core router"</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>trim / trim_start / trim_end</UKbd><span class="text-sm text-(--ui-text-muted)">清除模板或 Excel 中的多余空白</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>replace</UKbd><span class="text-sm"><code>from=" ", to="_"</code> 替换空格，生成合法标识符</span>
							</div>
						</div>
					</UPageList>
				</section>
					...
					<UPageList>
						<header class="space-y-1">
							...
							<p>
								...
								<code v-pre>{% for %}</code> 循环本身<strong>不要</strong>产生多余的空行。
							</p>
						</header>
						<p class="mt-2 text-sm">
							使用 <code v-pre>{%- ... -%}</code> 或 <code v-pre>{{- ... -}}</code> (注意那个减号 <code v-pre>-</code>) 来“吃掉”前后的空白（包括换行符）。
						</p>
						...
						<UAlert color="warning" variant="soft" class="mt-3">
							<template #description>
								<strong>强烈推荐</strong>：在 <code v-pre>{% for %}</code>、<code v-pre>{% if %}</code> 和 <code v-pre>{% endfor %}</code>
								这类控制标签上总是使用 <code v-pre>{%- ... -%}</code>，避免配置中出现意外的空行。
							</template>
						</UAlert>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<p>
							使用 <code v-pre>{# ... #}</code> 添加的注释<strong>不会</strong>出现在最终生成的配置中，仅用于模板维护。
						</p>
						<TemplateBatchCodeSnippet
							class="mt-2"
						...
						<div class="mt-2 space-y-2 text-sm">
							<div class="flex items-center gap-2">
								<UKbd>{# 模板注释 #}</UKbd><span class="text-(--ui-text-muted)">用于解释模板逻辑，最终输出时消失</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>! 或 #</UKbd><span>网设配置注释，会正常输出到配置中</span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					...
					<p class="mt-2 text-xs">
						Excel 中需为 <code>vlans</code> 列提供 JSON 数组，例如：
					</p>
				...
				<section>
					<UPageList>
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>{% if %}</UKbd><span class="text-sm">条件为真时执行</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>{% elif %}</UKbd><span class="text-sm">额外条件检查</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>{% else %}</UKbd><span class="text-sm">以上条件都不满足时执行</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>{% endif %}</UKbd><span class="text-sm text-(--ui-text-muted)">结束条件块</span>
							</div>
						</div>
						<TemplateBatchCodeSnippet class="mt-3" :code="snippets.condition" />
						<UAlert color="neutral" variant="subtle" class="mt-3">
							<template #description>
								Excel 中的 <code>device.role</code> 建议填 <code>core</code>、<code>distribution</code> 等枚举值
							</template>
						</UAlert>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<p>
							使用逻辑操作符构建更复杂的 <code v-pre>{% if %}</code> 判断，灵活处理不同场景。
						</p>
						<TemplateBatchCodeSnippet
							class="mt-2"
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>and</UKbd><span class="text-sm">两个条件必须同时满足</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>or</UKbd><span class="text-sm">满足任意一个条件即可</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>not</UKbd><span class="text-sm text-(--ui-text-muted)">条件不成立时执行</span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<p class="text-xs mt-3">
							如果需要跨板卡（如 1/0/x 与 3/0/x），可以嵌套循环：
						</p>
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>device.port_count</UKbd><span class="text-sm">填写端口总数，如 <code>42</code> 生成 1-42 连续接口</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>slot_range</UKbd><span class="text-sm text-(--ui-text-muted)">自定义板卡列表或上下限，适合 1/0/x ~ 3/0/x 这类结构</span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>interface.speed</UKbd><span class="text-sm">主要字段，为空时使用回退值</span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>interface.max_speed</UKbd><span class="text-sm text-(--ui-text-muted)">备用字段</span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					...
					<UAlert color="neutral" variant="subtle" class="mt-3">
						<template #description>
							例如需要为多台 PE 设备生成相同格式的接口/BGP 邻居配置时，只需在 Excel 中列出参数，模板里调用宏即可减少复制粘贴
						</template>
					</UAlert>
				</section>
				...
				<section>
					<UPageList>
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>device.vlan_base</UKbd><span class="text-sm">如 <code>100</code></span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>device.gw_offset</UKbd><span class="text-sm text-(--ui-text-muted)">如 <code>1</code> (即 .1 做网关)</span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<p>
							...
							<code v-pre>.txt</code>
							文件，然后在主模板中使用 <code v-pre>{% include "..." %}</code> 导入它们。
						</p>
						<TemplateBatchCodeSnippet
							class="mt-2"
						...
						<UAlert color="neutral" variant="subtle" class="mt-3">
							<template #description>
								这对于维护全局 ACL、标准安全策略或设备基础配置非常有用，实现“一处修改，处处生效”。
							</template>
						</UAlert>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>format</UKbd><span class="text-sm">格式化输出，如 <code v-pre>{{ vlan.id | format(value="%03d") }}</code></span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>trim</UKbd><span class="text-sm text-(--ui-text-muted)">清理空白字符，<code v-pre>{{ desc | trim }}</code></span>
							</div>
						</div>
					</UPageList>
				</section>
				...
				<section>
					<UPageList>
						...
						<div class="mt-2 space-y-2">
							<div class="flex items-center gap-2">
								<UKbd>device.role</UKbd><span class="text-sm">设备角色枚举：<code>core</code>、<code>distribution</code>、<code>access</code></span>
							</div>
							<div class="flex items-center gap-2">
								<UKbd>default</UKbd><span class="text-sm text-(--ui-text-muted)">结合 fallback 字段避免空值</span>
							</div>
						</div>
					</UPageList>
				</section>
